      ...
      textoSalir(Fuente::getFuente(IDFuente::TextoBotonMenu))
{
    ...
    if (ranking.empty()) {
        textosRanking.push_back("Aún no hay ganadores registrados.");
    } else {
        for (size_t i = 0; i < ranking.size(); ++i) {
            if (i == 5) break;
std::string texto = std::to_string(i + 1) + "° " + 
                                ranking[i].nombre + 
                                " - " + std::to_string(ranking[i].tiradas) + " tiradas - " + 
                                std::to_string(ranking[i].vida) + " vidas";
            textosRanking.push_back(texto);            
        }
    }
    ...
    textoTitulo.setString("Estadisticas");
    ...
    for (size_t i = 0; i < textosRanking.size(); ++i) {
        ...
        texto.setString(textosRanking[i]);
    ...
    Accesibilidad::hablar("Pantalla de estadísticas. Presiona los números del 1 al " + std::to_string(textosRanking.size()) + " para leer cada estadística. Presiona Escape para volver.");
}
...
EstadoID PantallaEstadisticas::manejarEventos(const sf::Event& evento) {
    if (const auto* keyPressed = evento.getIf<sf::Event::KeyPressed>()) {
        ...
        if (numPresionado != -1 && numPresionado < static_cast<int>(textosRanking.size())) {
            Accesibilidad::hablar(textosRanking[numPresionado]);
        }
    }
